...
# Example from github
imdb_data = find_by_title("Game of Thrones", type="series", season=1, episode=1)
...
# Seaon 1
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=1, episode=i))
}

# Season 2
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=2, episode=i))
}

# Season 3
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=3, episode=i))
}

# Season 4
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=4, episode=i))
}

# Season 5
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=5, episode=i))
}

# Season 6
for(i in 1:10){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=6, episode=i))
}

# Season 7
for(i in 1:7){
  imdb_data = dplyr::bind_rows(imdb_data, find_by_title("Game of Thrones", type="series", season=7, episode=i))
}

save(imdb_data, file = '/Users/alval/Box/Coursework/IndStudy/Data/imdb_data.RData')
```

Normally, I would create a nested for loop but the `omdbapi` package gives errors when data is scraped too quickly so instead I just created individual for loops for each season and waited a minute before running each one. There are some ways to make R pause inside of the loop but for simplicity I've done it this way.
...
get_time.com_deaths <- function(){
  
  # Obtain who died first
  who = xml2::read_html('http://time.com/3924852/every-game-of-thrones-death/') %>% 
  ...
  how = xml2::read_html('http://time.com/3924852/every-game-of-thrones-death/') %>% 
    rvest::html_nodes('p') %>% 
    rvest::html_text() %>% 
    dplyr::as_tibble() %>% 
  ...
  time = xml2::read_html('http://time.com/3924852/every-game-of-thrones-death/') %>% 
    rvest::html_nodes('p') %>% 
    rvest::html_text() %>% 
    dplyr::as_tibble() %>% 
...
time.com_death_data = get_time.com_deaths()
head(time.com_death_data)

save(time.com_death_data, file = "/Users/alval/Box/Coursework/IndStudy/Data/time.come_death_data.RData")
```

```{r,echo = FALSE}
load("/Users/alval/Box/Coursework/IndStudy/Data/time.come_death_data.RData")
head(time.com_death_data)
```
